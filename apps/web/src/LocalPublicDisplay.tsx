...
export function LocalPublicDisplay({ sessionId, go }: { sessionId: string; go: (path: string) => void }) {
  ...
  else if (snapshot.stage === 'clues') content = <div className="local-display-grid"><section><span className="local-kicker">ROUND {snapshot.roundNumber} · CLUE {snapshot.currentTurn + 1}/{snapshot.turnOrder.length}</span><h1>{current?.name ?? 'Next player'} is up.</h1><p>Listen closely. Every clue matters.</p><WordBoard words={snapshot.board} size={snapshot.boardSize} /></section><PublicClues snapshot={snapshot} /></div>;
  ...
  } else if (snapshot.stage === 'result' && snapshot.result) content = <section className="local-display-result"><img src="/moley-mascot.png" alt="Moley mascot" /><span className="local-kicker">ROUND {snapshot.roundNumber} COMPLETE</span><h1>{snapshot.result.headline}</h1><p>The secret word was</p><strong className="local-display-secret">{snapshot.result.secretWord}</strong><DisplayScores players={ranked} /></section>;
