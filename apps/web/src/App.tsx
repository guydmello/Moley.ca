...
function Lobby({ display, onShare, onSettings }: { display: boolean; onShare: () => void; onSettings: () => void }) {
  ...
  return <div className="lobby-layout"><section className="lobby-primary"><span className="game-kicker">THE TUNNEL IS OPEN · {room.settings.preset.toUpperCase()}</span><h1>{display ? 'Scan to join' : 'Waiting for suspicious people…'}</h1><p>{display ? 'Open your camera and point it here.' : 'Share the code. No account, no download, straight into the game.'}</p>{room.settings.showIcebreakers && <div className="icebreaker"><Sparkles /><span>LOBBY ICEBREAKER</span><strong>What harmless thing are you irrationally suspicious of?</strong></div>}<div className="room-share-card"><QRCodeSVG value={`${location.origin}/join/${room.code}`} size={display ? 240 : 142} bgColor="transparent" fgColor="currentColor" level="M" /><div><span>ROOM CODE</span><strong>{formatCode(room.code)}</strong><button className="button button-dark" onClick={onShare}><Share2 /> Share room</button></div></div><p className="config-summary">{room.settings.clueMode} clues · {room.settings.boardEnabled ? `${room.settings.boardSize}×${room.settings.boardSize} board · ` : ''}{room.settings.moleCount ?? 'Auto'} Mole{room.settings.moleCount === 1 ? '' : 's'} · {room.settings.targetScore ? `First to ${room.settings.targetScore}` : 'Endless'}{room.settings.chaosMode ? ' · Chaos on' : ''}</p>{large && <div className="large-warning"><Clock3 /> Spoken turns can take a while with {seats.length} players. Try Big Group mode.</div>}</section>
