...
import { APP_VERSION, defaultSettings, featureEnabled, normalizeRoomCode, settingsForPreset, type DrawingPayload, type GameSettings, type PublicPlayer } from '@moley/shared';
...
function sharedPackFromUrl(): string[] { const encoded = new URLSearchParams(location.search).get('pack'); if (!encoded) return []; try { const decoded = decodeURIComponent(escape(atob(encoded))); const words = JSON.parse(decoded) as unknown; return Array.isArray(words) ? words.filter((word): word is string => typeof word === 'string').slice(0, 100) : []; } catch { return []; } }
...
// Retained temporarily for persisted v2 drafts while the expanded dialog ships.
void SettingsDialog;
