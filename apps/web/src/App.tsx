...
import {
  ArrowRight, Bot, Check, Clock3, Copy,
...
const LocalPublicDisplay = lazy(() => import('./LocalPublicDisplay').then((module) => ({ default: module.LocalPublicDisplay })));
...
type Route = { page: 'home' | 'how' | 'join' | 'play' | 'display' | 'local' | 'local-display' | 'pass'; code?: string };
...
function routeFromPath(): Route {
  ...
  if (first === 'local-display') return { page: 'local-display', code: /^[a-z0-9-]{8,100}$/i.test(second ?? '') ? second : '' };
...
export function App() {
  ...
  if (route.page === 'local-display') return <Suspense fallback={<div className="local-display-page" />}><LocalPublicDisplay sessionId={route.code ?? ''} go={go} /></Suspense>;
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section className="faq wrap"><div className="section-kicker">QUESTIONS, ANSWERED</div><h2>Before you start digging.</h2><div className="faq-list">
        ...
        <details><summary>Do players need an account?<Plus /></summary><p>No. Pick a room-only display name and join with the four-word code, link, or QR.</p></details>
...
function CreateJoinDialog({ mode, onClose, go }: { mode: 'create' | 'join'; onClose: () => void; go: (path: string) => void }) {
  ...
  return <motion.div className="dialog-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && onClose()}><motion.div className="game-dialog" initial={{ y: 35, scale: .98 }} animate={{ y: 0, scale: 1 }}>
    ...
    <form onSubmit={submit}>
      ...
      {mode === 'join' && <label className="field"><span>Four-word room code</span><input value={code} onChange={(e) => setCode(e.target.value)} placeholder="PEACH FROG STAR BAY" autoCapitalize="characters" required /></label>}
...
function Lobby({ display, onShare, onSettings }: { display: boolean; onShare: () => void; onSettings: () => void }) {
  ...
  return <div className="lobby-layout"><section className="lobby-primary"><span className="game-kicker">THE TUNNEL IS OPEN · {room.settings.preset.toUpperCase()}</span><h1>{display ? 'Scan to join' : 'Waiting for suspicious people…'}</h1><p>{display ? 'Open your camera and point it here.' : 'Share the code. No account, no download, straight into the game.'}</p>{room.settings.showIcebreakers && <div className="icebreaker"><Sparkles /><span>LOBBY ICEBREAKER</span><strong>What harmless thing are you irrationally suspicious of?</strong></div>}<div className="room-share-card"><QRCodeSVG value={`${location.origin}/join/${room.code}`} size={display ? 240 : 142} bgColor="transparent" fgColor="currentColor" level="M" /><div><span>ROOM CODE</span><strong>{formatCode(room.code)}</strong><button className="button button-dark" onClick={onShare}><Share2 /> Share room</button></div></div><p className="config-summary">{room.settings.clueMode} clues · {room.settings.requiredClueRoundsBeforeVoting} clue round{room.settings.requiredClueRoundsBeforeVoting === 1 ? '' : 's'} · {room.settings.boardEnabled ? `${room.settings.boardSize}×${room.settings.boardSize} board · ` : ''}{room.settings.moleCount ?? 'Auto'} Mole{room.settings.moleCount === 1 ? '' : 's'} · {room.settings.targetScore ? `First to ${room.settings.targetScore}` : 'Endless'}{room.settings.chaosMode ? ' · Chaos on' : ''}</p>{large && <div className="large-warning"><Clock3 /> Spoken turns can take a while with {seats.length} players. Try Big Group mode.</div>}</section>
...
function MoleGuess() { const { room, send } = useGame(); const [guess, setGuess] = useState(''); return <div className="phase-screen mole-guess"><Eye size={46} /><span className="game-kicker">FINAL CHANCE</span><h1>Steal the word.</h1><p>You were caught—but one correct guess earns you a point.</p>{room?.settings.boardEnabled && <WordBoard words={room.board} size={room.settings.boardSize} label="Word board for your final guess" onSelect={(word) => setGuess(word.display)} />}<div className="guess-box"><input autoFocus value={guess} onChange={(e) => setGuess(e.target.value)} maxLength={80} placeholder="What was the secret word?" /><button className="button button-orange" disabled={!guess.trim()} onClick={() => send({ type: 'submit_mole_guess', guess })}>Lock guess <Lock /></button></div></div>; }

function OnlineBoard({ label }: { label: string }) { const room = useGame((state) => state.room); return room?.settings.boardEnabled ? <div className="online-board"><WordBoard words={room.board} size={room.settings.boardSize} label={label} /></div> : null; }
...
function RoundReveal() { const { room, me, send } = useGame(); if (!room?.result || !me) return null; return <div className="reveal-screen"><span className="game-kicker">GAME ROUND {room.roundNumber} COMPLETE</span><div className="reveal-mascot"><img src="/moley-mascot.png" alt="Moley mascot" /></div><h1>{room.result.headline}</h1>{Object.entries(room.result.moleGuesses).map(([id, guess]) => <p key={id}><strong>{room.players.find((player) => player.id === id)?.name}'s Final Guess:</strong> {guess}</p>)}<p>The secret word was</p><div className="secret-word-reveal">{room.result.secretWord}</div><div className="result-players">{room.result.moleIds.map((id) => { const player = room.players.find((item) => item.id === id); return <span key={id}><Eye /> {player?.name} was a Mole · +{room.result!.gains[id] ?? 0}</span>; })}</div>{me.canHost ? <button className="button button-primary button-xl" onClick={() => send({ type: 'host_advance' })}>See round recap <ArrowRight /></button> : <div className="waiting-host"><span className="live-dot" /> Waiting for host</div>}</div>; }
...
function DisplayPage({ code, go }: { code: string; go: (path: string) => void }) { const connect = useGame((state) => state.connect); const room = useGame((state) => state.room); const [error, setError] = useState(''); useEffect(() => { const open = async () => { const storageId = `public-display-v1-${code}`; const stored = restoreSession(storageId); if (stored) { connect(stored); return; } try { const response = await fetch(`/api/rooms/${code}/join`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: `Display ${crypto.randomUUID().slice(0, 6)}`, spectator: true, display: true }) }); const data = await response.json() as any; if (!response.ok) throw new Error(data.error); connect({ code: data.code, playerId: data.playerId, token: data.sessionToken, storageId }); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Display unavailable.'); } }; void open(); }, [code, connect]); if (error) return <CenteredPage go={go}><div className="standalone-card"><h1>{error}</h1><button className="button button-primary" onClick={() => go('/')}>Back home</button></div></CenteredPage>; if (!room) return <LoadingRoom go={go} />; return <Suspense fallback={<LoadingRoom go={go} />}><GameShell go={go} display /></Suspense>; }

function HowTo({ go }: { go: (path: string) => void }) {
  return <div className="rules-page"><nav className="site-nav wrap"><Brand go={go} /><button className="button button-primary" onClick={() => go('/')}><Play /> Play now</button></nav><main className="rules-wrap">
    <div className="rules-hero"><span className="section-kicker">THE TWO-MINUTE GUIDE</span><h1>How to play Moley</h1><p>Find the Mole without revealing the word they’re trying to steal. In Board Play, everyone also sees the same 5×5–10×10 candidate board.</p></div>
    <section className="rules-roles"><article><div className="rules-icon innocent"><ShieldCheck /></div><h2>If you know the word</h2><p>Give a clue that proves you know it, but stays subtle enough that the Mole cannot figure it out.</p><div className="example"><span>SECRET WORD · APPLE</span><strong>Good clue: “orchard”</strong><small>Risky clue: “red fruit”</small></div></article><article><div className="rules-icon mole"><Eye /></div><h2>If you’re the Mole</h2><p>Listen to every clue, act like you belong, and infer the secret before the group catches you.</p><div className="example dark"><span>YOU SEE · THE CATEGORY</span><strong>Blend in. Stay broad.</strong><small>If caught, guess the word for +1.</small></div></article></section>
    <section className="round-flow"><span className="section-kicker">ONE ROUND</span><h2>Clue. Discuss. Vote. Reveal.</h2><ol><li><b>1</b><div><strong>Reveal privately</strong><p>Only look at your own screen. A shared board is public.</p></div></li><li><b>2</b><div><strong>Give one clue</strong><p>Spoken or typed, in a new fair random order each round.</p></div></li><li><b>3</b><div><strong>Talk it out</strong><p>Question clues that felt too broad.</p></div></li><li><b>4</b><div><strong>Vote secretly</strong><p>Pick one suspect. No self-votes.</p></div></li><li><b>5</b><div><strong>Reveal and score</strong><p>A caught Mole must make one final guess.</p></div></li></ol><p>Playing locally? Pass the phone only on a neutral handoff screen. Open TV Display from Host Controls for a public board and scores; the TV never receives roles, the word, or private votes.</p></section>
    <section className="rules-scoring"><span className="section-kicker light">SCORING</span><h2>First to 5 wins.</h2><div><ScoreCard result="MOLE ESCAPES" mole="+2" team="0" tone="dark" /><ScoreCard result="CAUGHT + RIGHT GUESS" mole="+1" team="0" tone="orange" /><ScoreCard result="CAUGHT + WRONG GUESS" mole="0" team="+2 each" tone="lime" /></div><p>With multiple Moles, each one scores individually. Innocents earn points only if every Mole is caught and none guesses the word.</p></section>
    <section className="rules-cta"><img src="/moley-mascot.png" alt="Moley mascot" /><div><h2>Ready to look suspicious?</h2><p>No account. No download. One minute to start.</p></div><button className="button button-lime button-xl" onClick={() => go('/')}><Play /> Start digging</button></section>
  </main></div>;
}
