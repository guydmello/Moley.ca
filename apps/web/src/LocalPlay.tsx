...
export function LocalPlay({ go, initialPreset = 'local-classic' }: { go: (path: string) => void; initialPreset?: LocalPreset }) {
  ...
  useEffect(() => {
    const hidePrivateState = () => { delete document.documentElement.dataset.localPrivate; flushSync(() => { setRoleVisible(false); setVoteReady(false); }); };
  ...
  const update = (next: LocalGameState) => { delete document.documentElement.dataset.localPrivate; setRoleVisible(false); setVoteReady(false); setHumanClue(''); setBotThinking(false); if (next.settings.sound) localTone(); if (navigator.vibrate) navigator.vibrate(12); setNotice('Saved on this device'); setGame(next); setTimeout(() => setNotice(''), 1200); };
  const activePlayers = game?.players.filter((player) => player.active) ?? [];
  const secret = catalog.find((word) => word.id === game?.secretWordId);
  const board = game?.boardIds.map((id) => catalog.find((word) => word.id === id)).filter(Boolean) ?? [];

  const applyPreset = (preset: LocalPreset) => {
    const next = localSettingsForPreset(preset);
    setSettings({ ...next, customWords: settings.customWords, categories: settings.categories, sound: settings.sound });
  };
  const addBot = () => {
    const used = new Set([...names, ...bots.map((bot) => bot.name)]);
    const name = BOT_NAMES.find((candidate) => !used.has(candidate)) ?? `Bot ${bots.length + 1}`;
  ...
  const start = () => {
    ...
    try { update(startLocalRound(created, words, cryptoRandom)); }
    catch (error) { setNotice(error instanceof Error ? error.message : 'Could not start this local game.'); }
  };
  const fresh = async () => { await clearLocalGame(); setRecovery({ status: 'none' }); setGame(null); };
  ...
  const addBotNextRound = () => {
    ...
    const name = BOT_NAMES.find((candidate) => !used.has(candidate)) ?? `Bot ${game.players.filter((player) => player.kind === 'bot').length + 1}`;
  ...
  if (game.stage === 'voting') {
    ...
    return <div className="local-game">{header}<main className="local-stage"><section className="local-card local-private"><Lock /><span className="local-kicker">SECRET VOTING · BOT VOTES HIDDEN</span>{complete ? <><h1>All votes are locked.</h1><p>Bring everyone back before the reveal.</p><button className="button button-lime button-xl" onClick={() => update(resolveLocalVoting(game, catalog, cryptoRandom))}><Eye /> Reveal the vote</button></> : !voteReady ? <><h1>Pass to {voter?.name}</h1><p>Previous votes are hidden. Only {voter?.name} should look.</p><button className="button button-lime button-xl" onClick={() => setVoteReady(true)}><Lock /> I'm {voter?.name}</button></> : <><h1>Who is the Mole?</h1><p>Your vote stays hidden until everyone finishes.</p><div className="local-vote-grid">{activePlayers.filter((player) => player.id !== voter?.id).map((player) => <button key={player.id} onClick={() => { const next = submitLocalVote(game, voter!.id, player.id); update(next); setVoteReady(false); }}><span>{player.kind === 'bot' ? <Bot /> : player.name[0]}</span><strong>{player.name}</strong></button>)}</div></>}</section></main></div>;
  }

  if (game.stage === 'guess') {
    const accused = activePlayers.find((player) => game.accusedIds.includes(player.id));
    return <div className="local-game">{header}<main className="local-stage"><section className="local-card local-private"><Eye /><span className="local-kicker">ONE LAST CHANCE</span><h1>Pass to {accused?.name}</h1><p>The group caught the Mole. Choose the secret word privately to steal one point.</p><div className={`local-board size-${game.settings.boardSize}`}>{board.map((word) => <button key={word!.id} onClick={() => update(finishLocalRound(game, catalog, word!.id, cryptoRandom))}>{word!.display}</button>)}</div></section></main></div>;
  }

  if (game.stage === 'result') return <LocalResult game={game} secret={secret?.display ?? ''} goNext={() => update(startLocalRound(game, catalog, cryptoRandom))} />;
...
function LocalGameHeader({ game, online, notice, onPause, onRestart, onEnd, onSkip, onAddBot, onRemoveBot, onTimerChange }: { game: LocalGameState; online: boolean; notice: string; onPause: () => void; onRestart: () => void; onEnd: () => void; onSkip?: () => void; onAddBot?: () => void; onRemoveBot?: (id: string) => void; onTimerChange?: (seconds: number) => void }) {
  ...
  return <><header className="local-game-header"><span className="local-brand"><Eye /> MOLEY<span>.CA</span></span><div><span className="local-status"><Smartphone /> LOCAL GAME</span>{!online && <span className="local-status offline"><WifiOff /> OFFLINE</span>}<span>ROUND {game.roundNumber}</span></div><details><summary aria-label="Local host controls"><Settings2 /></summary><div><strong>HOST CONTROLS</strong><button onClick={onPause}>{game.paused ? <Play /> : <Pause />} {game.paused ? 'Resume' : 'Pause'}</button>{onSkip && ['clues', 'discussion'].includes(game.stage) && <button onClick={onSkip}><ArrowRight /> Skip / advance</button>}<button onClick={onRestart}><RefreshCw /> Restart round</button>{onAddBot && <button onClick={onAddBot}><Bot /> Add bot next round</button>}{onTimerChange && <><button onClick={() => onTimerChange(-15)}><Clock3 /> Timer −15s</button><button onClick={() => onTimerChange(15)}><Clock3 /> Timer +15s</button></>}{onRemoveBot && removableBots.map((bot) => <button key={bot.id} onClick={() => onRemoveBot(bot.id)}><X /> Remove {bot.name} next round</button>)}<button onClick={onEnd}><Trophy /> End match</button></div></details></header>{notice && <div className="local-notice" role="status">{notice}</div>}</>;
}
...
function LocalBoard({ board, size }: { board: ({ id: string; display: string } | undefined)[]; size: number }) {
  return <div className={`local-board size-${size}`} aria-label={`${size} by ${size} word board`}>{board.map((word) => <span key={word!.id}>{word!.display}</span>)}</div>;
}

function LocalResult({ game, secret, goNext }: { game: LocalGameState; secret: string; goNext: () => void }) {
  const ranked = [...game.players].filter((player) => player.active).sort((a, b) => b.score - a.score);
  return <div className="local-game"><LocalGameHeader game={game} online={navigator.onLine} notice="" onPause={() => undefined} onRestart={() => undefined} onEnd={() => undefined} /><main className="local-stage"><section className="local-card local-result"><img src="/moley-mascot.png" alt="Moley mascot" /><span className="local-kicker">ROUND {game.roundNumber} COMPLETE</span><h1>{game.result?.headline}</h1><p>The secret word was <strong>{secret}</strong>. {game.players.filter((player) => game.moleIds.includes(player.id)).map((player) => player.name).join(' & ')} {game.moleIds.length === 1 ? 'was' : 'were'} the Mole.</p><div className="local-scoreboard">{ranked.map((player, index) => <div key={player.id}><b>{index + 1}</b><span>{player.kind === 'bot' ? <Bot /> : player.name[0]}</span><strong>{player.name}</strong><small>{player.roundGain ? `+${player.roundGain}` : '—'}</small><em>{player.score} pts</em></div>)}</div><button className="button button-lime button-xl" onClick={goNext}>Start next round <ArrowRight /></button></section></main></div>;
}
