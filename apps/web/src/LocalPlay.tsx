...
import {
  ...
  createLocalPlayer, currentRolePlayer, finishLocalRound, localSettingsForPreset, lockLocalMoleGuess, playLocalClue,
...
const PRESETS: { id: LocalPreset; label: string; description: string; icon: typeof Smartphone }[] = [
  { id: 'local-bots', label: 'Quick', description: '1 clue round · First to 3', icon: Clock3 },
  { id: 'local-classic', label: 'Classic', description: '2 clue rounds · First to 5', icon: Users },
  { id: 'pass-the-phone', label: 'Detective', description: '3 clue rounds · First to 7', icon: Eye },
  { id: 'big-screen-party', label: 'Long Game', description: '4 clue rounds · First to 10', icon: Trophy },
  { id: 'offline-cottage', label: 'Five Rounds', description: '5 clue rounds · No timer', icon: ShieldCheck }
];
...
export function LocalPlay({ go, initialPreset = 'local-classic' }: { go: (path: string) => void; initialPreset?: LocalPreset }) {
  ...
  const [clueTransition, setClueTransition] = useState<{ completed: number; next: number | null } | null>(null);
  ...
  const update = (next: LocalGameState) => { delete document.documentElement.dataset.localPrivate; setRoleVisible(false); setVoteReady(false); setGuessReady(false); setHumanClue(''); setFinalGuess(''); setBotThinking(false); setClueTransition(null); if (next.settings.sound) localTone(); if (next.settings.haptics && navigator.vibrate) navigator.vibrate(12); setNotice('Saved on this device'); setGame(next); setTimeout(() => setNotice(''), 1200); };
  ...
  const applyPreset = (preset: LocalPreset) => {
    ...
    setSettings({ ...next, mode: settings.mode, customWords: settings.customWords, categories: settings.categories, sound: settings.sound, haptics: settings.haptics });
  };
  const addBot = () => {
  ...
  if (!game) {
    ...
    const setupRequirement = humans.length < 1
      ? 'Add at least one human player.'
      : seats < 4
        ? `Add at least ${4 - seats} more player${4 - seats === 1 ? '' : 's'} to start.`
        : '';
    return <LocalFrame go={go} online={online} notice={notice}><section className="local-setup">
      ...
      <div className="local-card">
        <h2>Choose game length</h2><div className="local-presets">{PRESETS.map(({ id, label, description, icon: Icon }) => <button key={id} className={settings.preset === id ? 'active' : ''} onClick={() => applyPreset(id)}><Icon /><span><strong>{label}</strong><small>{description}</small></span></button>)}</div>
        <h2>Choose presentation</h2><div className="local-presets"><button className={settings.mode !== 'party-board' ? 'active' : ''} onClick={() => setSettings({ ...settings, mode: 'pass-the-phone' })}><Smartphone /><span><strong>One Device</strong><small>Private handoffs and public play on this screen</small></span></button><button className={settings.mode === 'party-board' ? 'active' : ''} onClick={() => setSettings({ ...settings, mode: 'party-board' })}><Monitor /><span><strong>TV / Second Screen</strong><small>Secrets stay here; public play opens on the display</small></span></button></div>
        <div className="local-setup-grid"><div><h2>Who's playing?</h2><div className="local-name-list">{names.map((name, index) => <label key={index}><span>{index + 1}</span><input value={name} onChange={(event) => setNames(names.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} placeholder={`Player ${index + 1}`} maxLength={24} />{names.length > 1 && <button aria-label={`Remove player ${index + 1}`} onClick={() => setNames(names.filter((_, itemIndex) => itemIndex !== index))}><X /></button>}</label>)}</div><div className="local-add-row"><button className="button button-light" disabled={seats >= 100} onClick={() => setNames([...names, ''])}><Plus /> Human</button><button className="button button-light" disabled={seats >= 100} onClick={addBot}><Bot /> Add bot</button></div><div className="local-bot-list">{bots.map((bot) => <span key={bot.id}><Bot /> <strong>{bot.name}</strong><select aria-label={`${bot.name} difficulty`} value={bot.difficulty} onChange={(event) => setBots(bots.map((item) => item.id === bot.id ? { ...item, difficulty: event.target.value as LocalPlayer['difficulty'] } : item))}><option value="easy">Easy</option><option value="normal">Normal</option><option value="sneaky">Sneaky</option></select><button aria-label={`Remove ${bot.name}`} onClick={() => setBots(bots.filter((item) => item.id !== bot.id))}><X /></button></span>)}</div></div>
          <div><h2>Game setup</h2><div className="local-fields"><label><span>Board size</span><select value={settings.boardSize} onChange={(event) => setSettings({ ...settings, boardSize: Number(event.target.value) as LocalSettings['boardSize'] })}>{[5,6,7,8,9,10].map((size) => <option key={size} value={size}>{size} × {size}</option>)}</select></label><label><span>Clue rounds before voting</span><select aria-label="Clue rounds before voting" value={settings.requiredClueRoundsBeforeVoting} onChange={(event) => setSettings({ ...settings, requiredClueRoundsBeforeVoting: Number(event.target.value) as LocalSettings['requiredClueRoundsBeforeVoting'] })}>{[1,2,3,4,5].map((count) => <option key={count} value={count}>{count} {count === 1 ? 'round' : 'rounds'}</option>)}</select></label><label><span>First to</span><select value={settings.targetScore} onChange={(event) => setSettings({ ...settings, targetScore: Number(event.target.value) })}>{[3,5,7,10,15].map((score) => <option key={score} value={score}>{score} points</option>)}</select></label><label><span>Clue timer</span><select value={settings.clueSeconds} onChange={(event) => setSettings({ ...settings, clueSeconds: Number(event.target.value) })}><option value={0}>Off</option><option value={30}>30 seconds</option><option value={45}>45 seconds</option><option value={60}>60 seconds</option></select></label><label className="local-check"><input type="checkbox" checked={settings.fastBots} onChange={(event) => setSettings({ ...settings, fastBots: event.target.checked })} /> Fast Bots</label><label className="local-check"><input type="checkbox" checked={settings.sound} onChange={(event) => setSettings({ ...settings, sound: event.target.checked })} /> Sound</label><label className="local-check"><input type="checkbox" checked={settings.haptics} onChange={(event) => setSettings({ ...settings, haptics: event.target.checked })} /> Haptics</label></div><p className="config-summary">{settings.boardSize}×{settings.boardSize} board · <strong>{settings.requiredClueRoundsBeforeVoting} clue round{settings.requiredClueRoundsBeforeVoting === 1 ? '' : 's'} before voting</strong> · First to {settings.targetScore}</p><details className="local-packs"><summary>Custom local pack</summary><p>One word per line or comma-separated. Saved only on this device; humans can use any word, while bot secrets stay inside the curated bot library.</p><textarea value={customText} onChange={(event) => setCustomText(event.target.value)} placeholder={'Campfire\nCanoe\nMarshmallow'} /><button className="button button-light" onClick={() => { localStorage.setItem('moley:local:saved-pack', customText); setNotice('Custom pack saved on this device'); }}>Save pack locally</button></details></div></div>
        {setupRequirement && <p className="local-setup-validation" role="status">{setupRequirement}</p>}
        <button className="button button-lime button-wide button-xl" aria-label="Start game locally" onClick={start} disabled={Boolean(setupRequirement)}><Play /> Start Local Game</button>
      </div>
    </section></LocalFrame>;
  }

  const commitClue = (next: LocalGameState) => {
    const transition = next.stage === 'discussion'
      ? { completed: next.completedClueRounds, next: null }
      : next.stage === 'clues' && next.currentClueRound > game.currentClueRound
        ? { completed: next.completedClueRounds, next: next.currentClueRound }
        : null;
    update(next);
    if (transition) setClueTransition(transition);
  };

  const skip = () => {
    if (game.stage === 'clues') {
      const current = game.players.find((player) => player.id === game.turnOrder[game.currentTurn]);
      commitClue(playLocalClue(game, catalog, current?.kind === 'human' ? 'passed' : undefined, cryptoRandom));
  ...
  if (clueTransition) return <div className="local-game">{header}<main className="local-stage"><section className="local-card local-recovery"><Check /><span className="local-kicker">GAME ROUND {game.roundNumber}</span><h1>{clueTransition.next ? `Clue Round ${clueTransition.completed} Complete` : 'All clues are in'}</h1><p>{clueTransition.next ? 'Everyone gets another clue in the same turn order.' : 'Discuss who you think the Mole is before secret voting begins.'}</p><button className="button button-lime button-xl" onClick={() => setClueTransition(null)}>{clueTransition.next ? `Start Clue Round ${clueTransition.next}` : 'Start Discussion'} <ArrowRight /></button></section></main></div>;
  ...
  if (game.stage === 'clues') {
    ...
    const submit = (event: FormEvent) => { event.preventDefault(); try { commitClue(playLocalClue(game, catalog, humanClue, cryptoRandom)); } catch (error) { setNotice(error instanceof Error ? error.message : 'Clue could not be saved.'); } };
    const runBot = () => {
  ...
  if (game.stage === 'guess') {
    ...
    const botMole = accused?.kind === 'bot';
    return <div className="local-game">{header}<main className="local-stage"><section className="local-card local-private local-guess-private"><Eye /><span className="local-kicker">FINAL GUESS</span>{game.moleFinalGuess ? <><h1>Guess Locked</h1><p>The private guess is hidden. Put the device back where everyone can see it.</p><button className="button button-lime button-xl" onClick={() => update(finishLocalRound(game, catalog))}><Eye /> Reveal</button></> : botMole ? <><h1>{accused?.name} gets one final chance.</h1><p>The Bot Mole will guess only from public clues collected across every Clue Round.</p><button className="button button-lime button-xl" onClick={() => update(lockLocalMoleGuess(game, catalog, undefined, cryptoRandom))}><Bot /> Let {accused?.name} lock a guess</button></> : !guessReady ? <><h1>The Mole was caught.</h1><p>Pass the device to {accused?.name}. Only the Mole should make the Final Guess.</p><button className="button button-lime button-xl" onClick={() => { if (!history.state?.moleyPrivateReveal) history.pushState({ ...history.state, moleyPrivateReveal: true }, '', location.href); document.documentElement.dataset.localPrivate = 'revealed'; setGuessReady(true); }}><Lock /> I'm {accused?.name}</button></> : <form className="local-clue-form" onSubmit={(event) => { event.preventDefault(); if (history.state?.moleyPrivateReveal) history.back(); update(lockLocalMoleGuess(game, catalog, finalGuess, cryptoRandom)); }}><h1>What was the secret word?</h1><p>Enter one private guess. The secret stays hidden until everyone reveals it.</p><label><span>Final word guess</span><input autoFocus value={finalGuess} onChange={(event) => setFinalGuess(event.target.value)} maxLength={80} autoComplete="off" /></label><button className="button button-lime button-xl" disabled={!finalGuess.trim()}><Lock /> Lock In Guess</button></form>}</section></main></div>;
  }
...
function LocalResult({ game, secret, goNext, onOpenDisplay }: { game: LocalGameState; secret: string; goNext: () => void; onOpenDisplay: () => void }) {
  ...
  return <div className="local-game"><LocalGameHeader game={game} online={navigator.onLine} notice="" onOpenDisplay={onOpenDisplay} /><main className="local-stage"><section className="local-card local-result"><img src="/moley-mascot.png" alt="Moley mascot" /><span className="local-kicker">GAME ROUND {game.roundNumber} COMPLETE</span><h1>{game.result?.headline}</h1>{Object.entries(game.result?.moleGuesses ?? {}).map(([id, guess]) => <p key={id}><strong>{game.players.find((player) => player.id === id)?.name}'s Final Guess:</strong> {guess}</p>)}<p>The secret word was <strong>{secret}</strong>. {game.players.filter((player) => game.moleIds.includes(player.id)).map((player) => player.name).join(' & ')} {game.moleIds.length === 1 ? 'was' : 'were'} the Mole.</p><div className="local-scoreboard">{ranked.map((player, index) => <div key={player.id}><b>{index + 1}</b><span>{player.kind === 'bot' ? <Bot /> : player.name[0]}</span><strong>{player.name}</strong><small>{player.roundGain ? `+${player.roundGain}` : '—'}</small><em>{player.score} pts</em></div>)}</div><button className="button button-lime button-xl" onClick={goNext}>Start next Game Round <ArrowRight /></button></section></main></div>;
}
