...
async function runRoom(index: number) {
  ...
  const sockets = sessions.map((session) => new WebSocket(`${base.replace(/^http/, 'ws')}/api/rooms/${session.code}/connect?token=${encodeURIComponent(session.sessionToken)}`));
  ...
  const send = (socketIndex: number, event: Record<string, unknown>) => sockets[socketIndex]!.send(JSON.stringify({ ...event, v: 1, id: crypto.randomUUID(), seq: ++sequences[socketIndex]! }));
  ...
  if (scenario === 'reconnect-storm') {
    ...
    const replacements = sessions.map((session) => new WebSocket(`${base.replace(/^http/, 'ws')}/api/rooms/${session.code}/connect?token=${encodeURIComponent(session.sessionToken)}`));
